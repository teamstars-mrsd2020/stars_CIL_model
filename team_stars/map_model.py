...
def visualize(batch, out, between, out_cmd, loss_point, loss_cmd, target_heatmap):
    ...
    for i in range(out.shape[0]):
        ...
        _target_heatmap = np.uint8(
            target_heatmap[i].detach().squeeze().cpu().numpy() * 255
        )
...
class MapModel(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.net = SegmentationModel(
            10, 4, hack=hparams.hack, temperature=hparams.temperature
        )
    ...
    def training_step(self, batch, batch_nb):
        ...
        loss_point = torch.nn.functional.l1_loss(out, points, reduction="none").mean(
            (1, 2)
        )
        ...
        if batch_nb % 250 == 0:
            metrics["train_image"] = visualize(
                batch, out, between, out_cmd, loss_point, loss_cmd, target_heatmap
            )
    ...
    def validation_step(self, batch, batch_nb):
        ...
        loss_cmd_raw = torch.nn.functional.l1_loss(out_cmd, actions, reduction="none")
        loss_cmd_pred_raw = torch.nn.functional.l1_loss(
            out_cmd_pred, actions, reduction="none"
        )
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optim, mode="min", factor=0.5, patience=5, min_lr=1e-6, verbose=True
        )
...
def main(hparams):
    ...
    logger = WandbLogger(
        id=hparams.id, save_dir=str(hparams.save_dir), project="stage_1"
    )
