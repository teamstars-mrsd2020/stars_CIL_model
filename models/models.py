...
class SpatialSoftmax(torch.nn.Module):
    def forward(self, logit, temperature):
        ...
        x = (
            weights.sum(-2) * torch.linspace(-1, 1, logit.shape[-1]).type_as(logit)
        ).sum(-1)
        y = (
            weights.sum(-1) * torch.linspace(-1, 1, logit.shape[-2]).type_as(logit)
        ).sum(-1)
...
class SegmentationModel(torch.nn.Module):
    def __init__(
        self, input_channels=3, n_steps=4, batch_norm=True, hack=False, temperature=1.0
    ):
        super().__init__()
        ...
        self.network = deeplabv3plus_mobilenet(
            pretrained_backbone=False, num_classes=n_steps
        )
